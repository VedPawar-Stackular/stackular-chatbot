...
2. **`archive/deprecated-scripts/manual_reindex.py` uses `bge-large-en-v1.5` (1024-dim)** — running it against the runtime `bge-small-en` index (384-dim) will break dimension compatibility. It's kept only for reference; use `POST /admin/reindex` instead.
